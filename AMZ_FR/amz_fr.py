...
class Amz(object):
    ...
    def __init__(self, info_id):
        ...
        self.data_tool = getinfo_fr.GetInfo()
        # 注释这两行会导致最后输出结果的延迟，即等待页面加载完成再输出

    # 注册填入信息界面
    def register(self, y, x):
        # 获取代理
        chrome_options = getproxy.getpro()
        ...
        try:
            ...
            if 'Votre nom' in dr.page_source:
                ...
                time.sleep(10)
                if 'Saisir un code' in dr.page_source:
                    self.amil_code(dr, amil_num, login_wd)
                elif "sur l'image." in dr.page_source:
    ...
    def amil_code(self, dr, amil_num, login_wd):
        ...
        if vir_code == 1:
            ...
            bonjour_name = 'Bonjour ' + firstname
            if bonjour_name in html:
                ...
                # wait.until(EC.presence_of_element_located(
                #     (By.XPATH, '//*[@id="nav-search"]/form/div[2]')))
